...
## ⚠️ LOCAL ADAPTATION — READ THIS BEFORE ANYTHING ELSE

This is the upstream `gitroomhq/postiz-agent` skill, **modified for this repository**.
Everything below the "Local adaptation" section is upstream text, left untouched.
Three deviations from upstream apply to EVERY command in this document.

**Deviation 1 — There is no global `postiz` binary. Use the wrapper.**

Wherever this document writes `postiz <something>`, run `./scripts/postiz <something>`
from the repository root instead. The wrapper runs the CLI through `npx` at a pinned
version and loads credentials from `.env.postiz-cli`. Nothing is installed globally.
Do NOT run `npm install -g postiz`.

```bash
./scripts/postiz integrations:list        # correct
postiz integrations:list                  # wrong — command not found
```

**Deviation 2 — Never run `postiz auth:login`, `auth:status` or `auth:logout`.**

Those commands target the **Postiz cloud** (`cli-auth.postiz.com`, `api.postiz.com`),
not this self-hosted instance. Worse, `auth:login` writes `~/.postiz/credentials.json`,
which the CLI reads *with priority over* `POSTIZ_API_KEY` / `POSTIZ_API_URL` — every
later command would silently hit the cloud instead of our server.

Authentication here is file-based and already handled by the wrapper: `POSTIZ_API_KEY`
and `POSTIZ_API_URL` in `.env.postiz-cli`. Ignore the whole "Authentication" section
below and skip step 1 of the "Core Workflow". If a command returns 401, the fix is to
check `.env.postiz-cli`, never to run `auth:login`.

**Deviation 3 — AI generation commands do not work on this instance.**

This deployment has no `OPENAI_API_KEY`, so anything relying on server-side AI
(image generation, video generation, the `ask_postiz` agent) fails with a 500.
Media must be uploaded from an existing file via `./scripts/postiz upload <file>`.

Repository documentation: see the "CLI Postiz" section of the root `README.md`.

---

## Install Postiz if it doesn't exist

<!-- Local adaptation: upstream installs the CLI globally. Superseded by
     Deviation 1 above — use ./scripts/postiz, which calls npx with a pinned
     version. The upstream instructions are kept here for reference only.
...
| **allowed-tools** | Bash(./scripts/postiz:*) |
...
*(Local adaptation: handled by `.env.postiz-cli`, see Deviation 2. Nothing to run.)*
...
*Local adaptation: this whole section is superseded by Deviation 2. Credentials live
in `.env.postiz-cli` and the wrapper loads them. Do not run any `auth:*` command —
they all target the Postiz cloud. Verify the setup with a harmless read instead:*

```bash
./scripts/postiz integrations:list
```

*If that returns JSON, authentication works. If it returns 401, fix `POSTIZ_API_KEY`
in `.env.postiz-cli`. If it returns HTML, `POSTIZ_API_URL` is missing its `/api` suffix.
Upstream text follows, for reference only:*
...
```bash
# 1. Authenticate — SKIP on this repo, handled by .env.postiz-cli (Deviation 2).
#    Do not run auth:status / auth:login, they target the Postiz cloud.

# 2. Discover
./scripts/postiz integrations:list
./scripts/postiz integrations:settings <integration-id>
